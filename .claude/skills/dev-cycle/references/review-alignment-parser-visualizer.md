...
### Phase 5: Write to `internal/changes/visualizer/parser-output_REVISIONS_{NNN}.md`

Write the grouped plan to `internal/changes/visualizer/parser-output_REVISIONS_{NNN}.md` (create the `internal/changes/visualizer/` directory if needed). Use `_001` as the default sequence number; if `_001` already exists, increment to `_002`, etc.
