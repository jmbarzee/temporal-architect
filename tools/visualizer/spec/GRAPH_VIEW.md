...
A node's identity is a **dimension map** — `dimensions: Record<DimensionId, DimensionValue>` — rather than a single type string, and every feature elects exactly one axis to read. This domain projects onto two: `temporalType` and `sourceFile`. The entries describing each value of the type axis (`ladder`, `tier`, sizing, colour, the default Y band) live with the host in `NODE_TYPE_REGISTRY` (`src/adapter/node-types.ts`), the edge-side entries in `src/adapter/edge-types.ts`; the *shapes* they fill in are library-owned (`src/graph/taxonomy.ts`). Nothing in the graph engine imports either: it reaches its domain through an injected `Ontology` (`src/graph/ontology.ts`), and the physics params are keyed by dimension value and edge-type id. There is no numeric `level`/depth field on `GraphNode`: the earlier encoding that flattened both ladders' tiers into one sortable number has been removed, and consumers read the registry fields directly.
...
Each duplicate shares a stable `definitionKey` — opaque to the engine, derived by whoever produced the graph (here the parser's `definition` field, with the parent service name folded in for operations as `Service.Op`). The graph carries a `duplicateGroups: Map<definitionKey, Set<nodeId>>` index that the view uses during hover/select to keep sister copies visually grouped (see § Interaction States: Duplicate Highlighting). Definitions that are not registered on any worker remain orphan singletons (one node, no parent, `orphan: true`).

**Parameterized-family nodes (`templateParams`).** The container-tier nodes `namespace` and `nexusEndpoint` carry an optional `templateParams: string[]` wire field (`graph.Node.TemplateParams`, JSON key `templateParams`, **omitted when empty**). It names the template holes of a parameterized family — e.g. `["org"]` for `fabric-shard-{org}`. Its sole role in the view is presentation: it drives the **collapsed representative + cardinality badge** rendering (§ Visual Encoding → Node Appearance). It does **not** participate in identity. A parameterized family's `id` and `definitionKey` still carry the complete templated name string with holes (e.g. `namespace:fabric-shard-{org}`, `nexusEndpoint:fabric-shard-{org}-BootstrapShard`), so cross-view selection, `duplicateGroups`, and the bare/flat-global identity of `namespace` and `nexusEndpoint` (§ Packages and Qualified Identity — these node types keep bare, flat-global identity) are all unaffected by the presence of holes. Because the field is omitted when empty, static `namespace`/`nexusEndpoint` nodes carry **byte-identical** wire to before this feature existed and render exactly as they do today (no badge, no collapse); the feature is purely additive.
...
**Parameterized-family nodes (cardinality badge).** A `namespace` or `nexusEndpoint` node whose wire carries a non-empty `templateParams: string[]` — a parameterized family such as `fabric-shard-{org}` — renders as a **single collapsed representative node**: the same shape, size, and colour as its static counterpart (rounded rectangle / large for a namespace; the endpoint container treatment for a `nexusEndpoint`), adorned with a **cardinality badge**. The badge is derived **only** from `templateParams`: one `× {param}` chip per entry — `× org` for `["org"]`; `× org` and `× region` for `["org","region"]` — so the chip count equals the array length and each chip label is the bare param name. A family with two holes shows two chips. The badge is a node **adornment**, sitting alongside the existing badge vocabulary (the `?` unresolved badge and the orphan / "unassigned" badge — see [PRODUCT.md](./PRODUCT.md) § Node badges and § Orphan Definitions), **not** the node's name label: the label continues to show the full name string with holes included, subject to the same 15/12-character elision (§ Node Appearance). The hover tooltip's context line for these node types (§ Interaction States → Context line format) is unchanged in structure — the parameterized nature is already legible from the badge and the templated name.

*Static fallback.* When `templateParams` is absent (the field is omitted when empty), a `namespace`/`nexusEndpoint` node gets **no badge and no collapse treatment** — byte-identical wire, identical rendering to today. The feature is purely additive.

*Deferred — ghost-node depth.* Per-instance ghost-node / expansion depth — showing a family's individual members (`fabric-shard-acme`, `fabric-shard-globex`, …) as their own nodes — is out of scope this cycle; a family renders as the collapsed representative + badge only. Deferred to [#129](https://github.com/jmbarzee/temporal-architect/issues/129).
