...
`nodeType` is the single axis of identity in the implementation. All per-node-type metadata — `ladder` (main/nexus), `tier` (container/host/orchestrator/leaf), sizing, colour, and the default Y band — lives in `NODE_TYPE_REGISTRY` (`src/graph/node-types.ts`), with the edge-side equivalent in `EDGE_TYPE_REGISTRY` (`src/graph/edge-types.ts`); the physics params are keyed by node type and edge-type id. There is no numeric `level`/depth field on `GraphNode`: the earlier encoding that flattened both ladders' tiers into one sortable number has been removed, and consumers read the registry fields directly.
