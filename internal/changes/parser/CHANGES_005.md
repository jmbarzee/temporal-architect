...
**Source review(s):** `parser/REVISIONS_005.md` (promoted from the chunks workstream; the decomposition design-of-record is now `tools/lsp/parser/decompose/README.md`, with forward work tracked as issues [#38](https://github.com/jmbarzee/temporal-architect/issues/38)–[#45](https://github.com/jmbarzee/temporal-architect/issues/45)). Un-defers the basic case of *Graph Decomposition: Composable Chunks / Workflow Trees*, **without** depending on declared inbound roots ([#42](https://github.com/jmbarzee/temporal-architect/issues/42)).
**REVISIONS file(s):** `parser/REVISIONS_005.md` (consumed and removed — the dev cycle deletes a REVISIONS file once its CHANGES record lands)
...
- **`twf graph chunks`** (new CLI subcommand; `tools/lsp/cmd/twf/chunks.go`): additive — no existing contract changes. Thin wrapper that runs the existing parse → resolve → `graph.Extract` (or `history.Build` under `--history`), then `decompose.Decompose`, wrapped in the standard JSON envelope under a new `chunks` payload key. Flags: `--json`, `--ceiling N`, `--floor M`, `--by tree,nexus,worker,namespace`, `--history <dir>`. Routed from `graphCommand` when the first arg is `chunks` (`tools/lsp/cmd/twf/graph.go`). Definition-collapse stays internal — no `twf graph defs` lens this pass.
  ...
  > **Correction (later):** the `--history <dir>` flag (and the `history.Build` path behind it) was removed when history extraction moved into the sampler and the parser became Temporal-free — the sampler emits `observed-graph.json` instead; restoring a history input path for decomposition is [#65](https://github.com/jmbarzee/temporal-architect/issues/65).
- **`decompose.Decompose(file *ast.File, g *graph.Graph, opts Options) *Result`** (new public entry; `tools/lsp/parser/decompose/decompose.go`): the package's only exported function, plus the exported result model (`Result`, `Node`, `Root`, `Chunk`, `ChunkRoot`, `ChunkEdge`, `Division`, `Section`, `SectionEdge`, `Options`) and constants (`SourceHeuristic`/`SourceDeclared`, `StrategyTree`/`Nexus`/`Worker`/`Namespace`, `DefaultFloor`). Roots carry `source: heuristic|declared` from day one so declared inbound roots ([#42](https://github.com/jmbarzee/temporal-architect/issues/42)) slot in additively.

### Schema

- **twf JSON wire contract** (then the hand-maintained `twf.schema.json`; the contract is now generated into `tools/wire-types/` via `make gen-types`, CI-gated by `make check-types`, and the hand-written schema file with its `$defs/Decomposition` family and `$schemaVersion` marker is gone): added the `chunks` top-level payload and the decomposition type family (`ChunkNode`, `ChunkRootDecl`, `Chunk`, `ChunkRootReach`, `ChunkEdge`, `Division`, `Section`, `SectionEdge`). Additive (envelope is `additionalProperties: true`); `$schemaVersion` bumped `1.0.0` → `1.1.0` (minor). No existing shapes changed.
...
- **`tools/lsp/parser/decompose/complexity.go`** (new): the deterministic AST-derived complexity scalar (documented, tunable weights — base, body statements, distinct call fan-out, branch/loop depth, handler count, child-workflow count; **not** a calibrated model) and the **floor** (`--floor`, default `2` at the time — **correction:** `DefaultFloor` was later retuned to `4`, with the rationale in the constant's own comment at `tools/lsp/parser/decompose/decompose.go:35`): a below-floor chunk is flagged too-granular and recommended for merge into the chunk that dispatches into it (over a contract edge). Recommend-only — never auto-applied.
...
- **Package overview.** *(Correction: the planned `decompose/doc.go` was never created — the package doc is inline at `tools/lsp/parser/decompose/decompose.go:1-9` and the architecture write-up is `tools/lsp/parser/decompose/README.md`.)*
...
- **Floor/ceiling defaults & metric weights:** shipped documented defaults — floor `2` (negative disables; **correction:** later retuned to `4`), no default ceiling (#2 is opt-in via `--ceiling`), weights as constants in `complexity.go`. To be tuned from real designs; not a calibrated model.
...
- **Spillover deferred**, now tracked as GitHub issues: the **loop cut ceiling** ([#38](https://github.com/jmbarzee/temporal-architect/issues/38)) and the standalone **worker/namespace/nexus grouping lens** ([#39](https://github.com/jmbarzee/temporal-architect/issues/39)). Additive future work (**declared inbound roots** and **language boundaries**, [#42](https://github.com/jmbarzee/temporal-architect/issues/42)) is designed to slot in without a pipeline reshape — roots already emit `source`, nodes already retain deployment attributes.
