...
**Source review(s):** a Go CLI library survey (cobra selected) — committed recommendation (**cobra**) and migration sketch.
...
([`tools/lsp/cmd/twf/COMMANDS.md`](../../../tools/lsp/cmd/twf/COMMANDS.md)) is
...
function and the envelope marshalling are byte-for-byte unchanged, and the
JSON wire contract is untouched (then a hand-maintained `twf.schema.json`; it is
now generated into `tools/wire-types/` via `make gen-types`, CI-gated by
`make check-types`).
...
- **`graph chunks` dispatch:** now a first-class cobra child command rather than
  ...
  **Correction (later):** the `--history` flag — and the `envelope.LoadHistories`
  / `HistoryDiagnostics` helpers described below — were removed when history
  extraction moved into the sampler and the parser became Temporal-free (the
  sampler emits `observed-graph.json` instead); restoring a history input path
  for decomposition is [#65](https://github.com/jmbarzee/temporal-architect/issues/65).
...
- **`vscode`** (the VS Code extension lives in the separate `jmbarzee/temporal-architect-dist` repo): consumes LSP responses and `twf parse`/
